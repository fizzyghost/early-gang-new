...
async def controller(message):
    ...
    if chatPlays.chatPlaying is True:
        ...
        lightPressTime = (random.randint(1, 3) / 400)
        holdTime = (random.randint(9, 15) / 10)
        ...
        try:
            ...
            # 25% chance of input
            if int(streamResponse.get("data")[0].get("viewer_count")) > 250:
                if dice <= 25:
                    input = True

            # 35% chance of input
            elif int(streamResponse.get("data")[0].get("viewer_count")) > 200:
                if dice <= 35:
                    input = True
					
            # 50% chance of input
            elif int(streamResponse.get("data")[0].get("viewer_count")) > 150:
                if dice <= 50:
                    input = True
